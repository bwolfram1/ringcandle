...
wick_pin = board.D5  # The data-in pin of the NeoPixel
...
strip = neopixel.NeoPixel(pixpin, numpix, brightness=0.5,
                          auto_write=True,pixel_order=ORDER )  # initialize strip
...
def set_color(index):
    ...
    for x in range(numpix):
        if index >= index_min:
            strip[x] = [index, int((index * 3) / 8), 0]
        elif index < index_min:
            strip[x] = [index, int((index * 3.25) / 8), 0]
